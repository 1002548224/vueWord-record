...
export default {
  ...
  methods: {
    ...
    showInput (row) {
      ...
      // 让文本自动获得焦点
      // $nextTick方法 的作用：当页面上的元素被重新渲染之后，才会执行回调函数中的代码
      this.$nextTick(_ => {
  ...
  computed: {
    ...
    isBtnDisable () {
      ...
      if (this.selectedCateKeys.length > 0) return false
      return true
    },
    // 当前选中的三级分类的id
    cateId () {
      if (this.selectedCateKeys.length > 0) return this.selectedCateKeys[this.selectedCateKeys.length - 1]
      return null
    },
    // 动态计算title文本
    titleText () {
